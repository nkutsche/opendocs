...
public final class ImageUtils {
	 ...
	 * @return the extracted sub-image
	 */
	public static BufferedImage newSubimage(BufferedImage src, int x, int y,
			int w, int h) {
	...
	public static BufferedImage recolor(BufferedImage img, int newColor) {
		...
		result.setRGB(0, 0, img.getWidth(), img.getHeight(), pixels, 0,
				img.getWidth());
		return result;
	}
	...
	public static BufferedImage[] splitByWidth(BufferedImage img, int width) {
		if (img.getWidth() > 0) {
			BufferedImage[] result = new BufferedImage[(img.getWidth() + width - 1)
					/ width];
			int x = 0;
	 ...
	 */
	public static BufferedImage[][] split(BufferedImage img, int width,
			int height) {
		if (img.getWidth() > 0) {

			BufferedImage[][] result = new BufferedImage[(img.getHeight()
					+ height - 1)
					/ height][(img.getWidth() + width - 1) / width];
			int y = 0;
			for (int j = 0; j < result.length; j++) {
				...
				for (int i = 0; i < result.length; i++) {
					int x2 = Math.min(x + width - 1, img.getWidth() - 1);
					result[j][i] = newSubimage(img, x, y, x2 - x + 1, y2 - y
							+ 1);
					x += width;
				}
				y += height;
